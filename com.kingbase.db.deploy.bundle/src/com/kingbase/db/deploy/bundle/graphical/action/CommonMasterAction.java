	...
	public void run() {
		...
		try {
			...
			if (type.equals("start")) {
				...
				if (entity.getLibrary().contains("库")) {// 重启库的时候需要重启pool
					String command1 =(flag ? stopPool(mainPool) + ";" : "") + startPool(mainPool);
					JschUtil.execCommand1(session, startDB(entity));
					JschUtil.execCommand1(sessionPool!= null ? sessionPool:session, command1);
				}
			} else if (type.equals("disable")) {

				 if (entity.getLibrary().contains("库")) {
					String command1 =(flag ? stopPool(mainPool) + ";" : "") + startPool(mainPool);
